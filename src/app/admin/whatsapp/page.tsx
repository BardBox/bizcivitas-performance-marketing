...
const EVENT_ICONS: Record<string, { icon: typeof MessageCircle; color: string }> = {
  sent: { icon: Send, color: "text-blue-500" },
  delivered: { icon: CheckCheck, color: "text-green-500" },
  read: { icon: Eye, color: "text-emerald-500" },
  failed: { icon: AlertTriangle, color: "text-red-500" },
};

const BUILT_IN_ICONS: Record<string, typeof MessageCircle> = {
  welcome: UserPlus,
  follow_up: ArrowRightLeft,
};

const BUILT_IN_DESCRIPTIONS: Record<string, string> = {
  welcome: "Automatically sent when a new inquiry is submitted (if phone number is provided)",
...
export default function WhatsAppPage() {
  ...
  const [topLeads, setTopLeads] = useState<WhatsappTopLead[]>([]);
  ...
  const [templateStats, setTemplateStats] = useState<TemplateStatsItem[]>([]);
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const [statsRes, eventsRes, leadsRes, templateStatsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/pm/whatsapp-tracking/stats?days=${days}`),
        fetch(`${API_BASE_URL}/pm/whatsapp-tracking/events?limit=100&sort=newest`),
        fetch(`${API_BASE_URL}/pm/whatsapp-tracking/top-leads?limit=10`),
        fetch(`${API_BASE_URL}/pm/whatsapp-tracking/template-stats`),
      ]);

      const [statsData, eventsData, leadsData, templateStatsData] = await Promise.all([
        statsRes.json(),
        eventsRes.json(),
        leadsRes.json(),
        templateStatsRes.json(),
      ]);

      if (statsData.statusCode === 200) setStats(statsData.data);
      if (eventsData.statusCode === 200) setRecentEvents(eventsData.data?.events || []);
      if (leadsData.statusCode === 200) setTopLeads(leadsData.data || []);
      if (templateStatsData.statusCode === 200) setTemplateStats(templateStatsData.data || []);
    } catch (err) {
      console.error("Failed to fetch WhatsApp reports:", err);
    } finally {
      setLoadingReports(false);
    }
  }, [days]);

  const handleSyncFromTft = async () => {
    setSyncing(true);
    setSyncResult(null);
    try {
      const res = await fetch(`${API_BASE_URL}/pm/whatsapp-tracking/sync`, {
        method: "POST",
      ...
      if (data.statusCode === 200) {
        setSyncResult({ synced: data.data.synced, total: data.data.total });
        // Refresh reports after sync
        await fetchReports();
      }
    } catch (err) {
      console.error("TFT sync failed:", err);
    } finally {
      setSyncing(false);
    }
  };
  ...
  const fetchAutomations = useCallback(async () => {
    ...
    try {
      const [autoRes, groupsRes, stagesRes, templatesRes] = await Promise.all([
        fetch(`${API_BASE_URL}/pm/whatsapp-automations`),
        fetch(`${API_BASE_URL}/pm/whatsapp-automations/groups`),
        fetch(`${API_BASE_URL}/pm/whatsapp-automations/pipeline-stages`),
  ...
  const handleSaveAutomation = async (
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/pm/whatsapp-automations`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          type,
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/pm/whatsapp-automations/groups`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteGroup = async (type: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/pm/whatsapp-automations/groups/${encodeURIComponent(type)}`,
        { method: "DELETE" }
      );
      const data = await res.json();
      if (data.statusCode === 200) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Automations, templates & message tracking
          </p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex items-center gap-1 mb-5 border-b border-gray-200 overflow-x-auto">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-3 text-sm font-medium text-gray-400 hover:text-[#25D366] border-b-2 border-transparent transition-colors cursor-pointer whitespace-nowrap"
            >
          ...
          <div className="mt-6 bg-green-50 border border-green-200 rounded-xl p-4">
            ...
            <p className="text-xs text-green-600">
              Templates must be pre-approved on the TFT/Meta platform before use.
              The <code className="bg-green-100 px-1 rounded">TFT Template Name</code> must
              exactly match the template name on the TFT dashboard. Create templates in the{" "}
              <a href="/admin/templates" className="underline font-medium">Templates</a> page.
            </p>
          </div>
        </>
      )}
      ...
      {mainTab === "reports" && (() => {
        const filteredEvents = recentEvents
          .filter((event) => {
            ...
            if (eventSearch.trim()) {
              ...
              const mobile = event.mobile?.toLowerCase() || "";
              const template = event.templateName?.toLowerCase() || "";
              if (!name.includes(q) && !mobile.includes(q) && !template.includes(q)) return false;
            }
            return true;
          })
          .sort((a, b) => {
        ...
        const statCards = [
          ...
          },
          {
            ...
            sub: stats?.deliveryRate ? `${stats.deliveryRate}% rate` : undefined,
          },
          {
            key: "read",
            label: "Read",
            ...
            sub: stats?.readRate ? `${stats.readRate}% rate` : undefined,
          },
          {
            key: "failed",
            label: "Failed",
        ...
        const hasActiveFilters = activeStatCard || eventFilter !== "all" || eventSearch;
              ...
              <div className="flex items-center gap-2">
                {syncResult && (
                  <span className="text-xs text-green-600 bg-green-50 px-2.5 py-1 rounded-full">
                    {syncResult.synced} new statuses synced
                  </span>
                )}
                <button
                  onClick={handleSyncFromTft}
                  disabled={syncing}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#25D366] text-[#25D366] hover:bg-[#25D366]/5 text-xs font-medium transition-colors cursor-pointer disabled:opacity-50"
                  title="Pull delivery & read statuses from TFT platform"
                >
                  {syncing ? (
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-semibold text-[#1a1a2e]">Per-Template Performance</h2>
                      </div>
                      <span className="text-[10px] font-medium text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                        {templateStats.length} templates
                      </span>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="text-[10px] uppercase tracking-wider text-gray-400 border-b border-gray-100">
                            ...
                            <th className="text-center py-2 px-2 font-medium">Read</th>
                        ...
                        <tbody>
                          {templateStats.map((t) => (
                            <tr key={t.templateName} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                              <td className="py-2.5 pr-4">
                                <span className="font-medium text-[#1a1a2e] text-xs">{t.templateName}</span>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full font-medium">{t.sent}</span>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-full font-medium">{t.delivered}</span>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full font-medium">{t.read}</span>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <span className="text-xs text-red-600 bg-red-50 px-2 py-0.5 rounded-full font-medium">{t.failed}</span>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <div className="flex items-center justify-center gap-1">
                                  <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                      ...
                                      style={{ width: `${Math.min(parseFloat(t.deliveryRate) || 0, 100)}%` }}
                                    />
                                  </div>
                                  <span className="text-[10px] text-gray-500 font-medium w-8">{t.deliveryRate}%</span>
                                </div>
                              </td>
                              <td className="text-center py-2.5 px-2">
                                <div className="flex items-center justify-center gap-1">
                                  <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                      ...
                                      style={{ width: `${Math.min(parseFloat(t.readRate) || 0, 100)}%` }}
                                    />
                                  </div>
                                  <span className="text-[10px] text-gray-500 font-medium w-8">{t.readRate}%</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                  ...
                  <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-5">
                    ...
                    {topLeads.length === 0 ? (
                      <div className="text-center py-10">
                        ...
                        <p className="text-sm text-gray-400">No WhatsApp data yet</p>
                        <p className="text-xs text-gray-300 mt-0.5">Send messages to see results</p>
                      </div>
                    ) : (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-medium text-[#1a1a2e] truncate max-w-[160px]">
                                  {lead.inquiryName || lead.mobile}
                                </p>
                                <p className="text-[10px] text-gray-400">
                                  {lead.inquiryName ? lead.mobile : ""} Last: {new Date(lead.lastEvent).toLocaleDateString("en-IN")}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2 text-xs">
                              <span className="flex items-center gap-1 text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full">
                                <Send className="w-3 h-3" /> {lead.sentCount}
                              </span>
                              <span className="flex items-center gap-1 text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">
                                <Eye className="w-3 h-3" /> {lead.readCount}
                              </span>
                            </div>
                          </div>
                        ))}
                  ...
                  <div className="lg:col-span-3 bg-white border border-gray-200 rounded-xl p-5">
                    ...
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <div className="relative flex-1 min-w-[180px]">
                        ...
                        <input
                          ...
                          placeholder="Search name, mobile, template..."
                          className="w-full pl-8 pr-3 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-[#25D366]/30 focus:border-[#25D366]"
                        />
                      </div>
                      <button
                    ...
                    <div className="flex items-center gap-1 mb-3">
                      {[
                        ...
                        { key: "read", label: "Read", color: "text-emerald-600 bg-emerald-50" },
                        { key: "failed", label: "Failed", color: "text-red-600 bg-red-50" },
                      ].map((f) => (
                        <button
                          key={f.key}
                    ...
                    {hasActiveFilters && (
                      <div className="flex items-center gap-2 mb-3 text-[10px] flex-wrap">
                        <Filter className="w-3 h-3 text-[#25D366]" />
                        <span className="text-gray-500">
                          Showing {filteredEvents.length} of {recentEvents.length} events
                          {activeStatCard && <span className="font-medium text-[#25D366]"> — {activeStatCard}</span>}
                        </span>
                        <button
                      ...
                      <div className="space-y-1 max-h-[420px] overflow-y-auto">
                        {filteredEvents.map((event) => {
                          ...
                          const typeBg = event.eventType === "sent" ? "bg-blue-50" :
                            event.eventType === "delivered" ? "bg-green-50" :
                            event.eventType === "read" ? "bg-emerald-50" :
                            event.eventType === "failed" ? "bg-red-50" : "bg-gray-50";
                          return (
                            <div
                              key={event._id}
                              className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-gray-50 transition-colors"
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="text-sm font-medium text-[#1a1a2e] truncate max-w-[200px]">
                                    {event.inquiryId?.fullName || event.mobile}
                                  </p>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <p className="text-[11px] text-gray-400 mt-1.5">
                  This will create a new WhatsApp automation tab with all pipeline stages.
                  Assign TFT templates to each stage.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#25D366] hover:bg-[#1da851] text-white text-sm font-medium transition-colors cursor-pointer disabled:opacity-70"
              >
                {creatingGroup && (
...
// ── WhatsApp Automation Row Component ──

function WaAutomationRow({
  stage,
  automation,
  templates,
  type,
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-1.5 min-w-[120px]">
          ...
          <input
            ...
            className="w-16 border border-gray-300 rounded-lg px-2 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-[#25D366]/30 focus:border-[#25D366]"
          />
      ...
      {automation?.templateId && (
        <p className="text-[11px] text-gray-400 mt-2 pl-[136px]">
          ...
          &middot; TFT: {automation.templateId.tftTemplateName}
        </p>
      )}
    </div>
  );
